...
<div class="min-h-screen bg-gray-50">
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-gray-600">Modify role settings and permissions</p>
                    </div>
            ...
            <form method="POST" action="{{ route('access-control.roles.update', $role) }}" x-data="roleForm()">
                @csrf
                @method('PUT')
                    ...
                    <div class="px-6 py-4 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required
                                       ...
                                       {{ $role->is_default ? 'readonly' : '' }}>
                                <p class="mt-1 text-xs text-gray-500">
                                    @if($role->is_default)
                                        This is a default role and the system name cannot be changed.
                                    @else
                                        Used internally by the system. Use lowercase with underscores.
                                    @endif
                                </p>
                            </div>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('description', $role->description) }}</textarea>
                        </div>
                        ...
                        @if($role->is_default)
                            <div class="bg-yellow-50 border border-yellow-200 rounded-md p-4">
                                <div class="flex">
                                    ...
                                    <div class="ml-3">
                                        <h3 class="text-sm font-medium text-yellow-800">Default Role</h3>
                                        <div class="mt-2 text-sm text-yellow-700">
                                            <p>This is a default role. Some settings may be restricted to prevent system instability.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
